/*!
 * @ai-native-solutions/fallbuild-sdk · v1.0.0
 * MIT · ai-nativesolutions.com
 *
 * Sovereign single-file low-code app builder — programmatic core.
 * Extracted verbatim from the fallbuild reference app.
 *
 * Public API:
 *   WIDGETS, DEMO_DATA, TEMPLATE_NAMES, TEMPLATE_KEYWORDS
 *   createApp(name)
 *   addPage(app, name)
 *   addWidget(app, pageIdx, type, x, y, w, h, props)
 *   applyTemplate(name)
 *   routePrompt(prompt)
 *   renderStandaloneHtml(app, opts)
 *   validateApp(app)
 *   mdToHtml(md)
 */

// ─── Widget palette (8 primitives) ──────────────────────────────────
export const WIDGETS = {
  ...
  table:     { label: 'Table',     icon: 'H', defaultProps: { dataSource: '', columns: '', sortable: true, rowAction: 'none', rowTarget: '' }, defaultSize: { w: 480, h: 240 } },
  ...
  filter:    { label: 'Filter',    icon: 'V', defaultProps: { placeholder: 'search...', target: '' },                                defaultSize: { w: 240, h: 48  } },
};
...
// ─── Template catalogue ─────────────────────────────────────────────
export const TEMPLATE_NAMES = ['CRUD app', 'Dashboard', 'Lead capture', 'Pricing calculator', 'Internal tool'];
...
export function createApp(name) {
  return {
    ...
    pages: [{ id: rid('p'), name: 'home', widgets: [] }],
    dataSources: [],
    createdAt: Date.now(),
    updatedAt: Date.now(),
  };
...
export function addPage(app, name) {
  const p = { id: rid('p'), name: name || 'page ' + (app.pages.length + 1), widgets: [] };
  app.pages.push(p);
  app.updatedAt = Date.now();
  return p;
}

export function mkWidget(type, x, y, w, h, props) {
...
export function addWidget(app, pageIdx, type, x, y, w, h, props) {
  const page = app.pages[pageIdx];
  if (!page) throw new Error('page index out of range: ' + pageIdx);
  const wg = mkWidget(type, x, y, w, h, props);
  page.widgets.push(wg);
  app.updatedAt = Date.now();
  return wg;
}

export function addDataSource(app, ds) {
  app.dataSources.push(ds);
  app.updatedAt = Date.now();
  return ds;
}

// ─── Template builder (layouts verbatim from reference app) ─────────
export function applyTemplate(name) {
  ...
  if (name === 'CRUD app') {
    ...
    base.pages = [
      { id: rid('p'), name: 'list', widgets: [
        mkWidget('text',   20, 20,  500, 40,  { content: '# Customers' }),
        mkWidget('button', 20, 70,  140, 36,  { label: '+ New customer', action: 'navigate', target: 'new' }),
        mkWidget('filter', 180,70,  200, 36,  { placeholder: 'search...' }),
        mkWidget('table',  20, 120, 720, 360, { dataSource: 'customers', columns: 'name,email,company,tier,value', sortable: true, rowAction: 'navigate', rowTarget: 'edit' }),
      ]},
      { id: rid('p'), name: 'new', widgets: [
        mkWidget('text',   20, 20,  500, 40,  { content: '# New customer' }),
        mkWidget('form',   20, 70,  400, 320, { dataSource: 'customers', title: 'Add customer', submitLabel: 'Save' }),
        mkWidget('button', 20, 400, 120, 36,  { label: 'back', action: 'navigate', target: 'list' }),
      ]},
      { id: rid('p'), name: 'edit', widgets: [
        mkWidget('text',   20, 20,  500, 40,  { content: '# Edit customer' }),
        mkWidget('form',   20, 70,  400, 320, { dataSource: 'customers', title: 'Edit', submitLabel: 'Update' }),
        mkWidget('button', 20, 400, 120, 36,  { label: 'back', action: 'navigate', target: 'list' }),
      ]},
    ];
    ...
    base.pages = [{ id: rid('p'), name: 'dashboard', widgets: [
      mkWidget('text',   20, 20,  800, 40,  { content: '# Dashboard' }),
      mkWidget('filter', 20, 70,  300, 36,  { placeholder: 'filter...' }),
      mkWidget('chart',  20, 120, 360, 240, { dataSource: 'orders',    chartType: 'bar',  xField: 'customer', yField: 'total' }),
      mkWidget('chart',  400,120, 360, 240, { dataSource: 'customers', chartType: 'pie',  xField: 'tier',     yField: 'value' }),
      mkWidget('chart',  20, 380, 740, 240, { dataSource: 'orders',    chartType: 'line', xField: 'date',     yField: 'total' }),
    ]}];
  } else if (name === 'Lead capture') {
    base.dataSources = [{ name: 'leads', type: 'inline', json: '[]' }];
    base.pages = [
      { id: rid('p'), name: 'capture', widgets: [
        mkWidget('text', 20, 20, 500, 80,  { content: '# Get a demo\nLeave your details and we will be in touch.' }),
        mkWidget('form', 20, 120,400, 260, { dataSource: 'leads', title: 'Your details', submitLabel: 'Request demo' }),
      ]},
      { id: rid('p'), name: 'success', widgets: [
        mkWidget('text', 20, 20, 500, 120, { content: '# Thank you\nWe got your details.' }),
      ]},
    ];
  } else if (name === 'Pricing calculator') {
    base.dataSources = [{ name: 'quotes', type: 'inline', json: '[]' }];
    base.pages = [{ id: rid('p'), name: 'calc', widgets: [
      mkWidget('text',   20, 20,  600, 80,  { content: '# Pricing calculator' }),
      mkWidget('form',   20, 120, 400, 260, { dataSource: 'quotes', title: 'Inputs', submitLabel: 'Calculate' }),
      mkWidget('text',   440,120, 300, 140, { content: '## Result\nFill the form to calculate.' }),
      mkWidget('button', 20, 400, 200, 40,  { label: 'custom calc', action: 'custom', code: 'alert("custom calc result: $" + Math.floor(Math.random()*1000))' }),
    ]}];
    ...
    base.pages = [
      { id: rid('p'), name: 'customers', widgets: [
        mkWidget('text',  20, 20, 400, 40,  { content: '# Customers' }),
        mkWidget('table', 20, 70, 760, 360, { dataSource: 'customers', columns: 'name,email,company,tier', sortable: true }),
      ]},
      { id: rid('p'), name: 'orders', widgets: [
        mkWidget('text',  20, 20, 400, 40,  { content: '# Orders' }),
        mkWidget('table', 20, 70, 760, 360, { dataSource: 'orders', sortable: true }),
      ]},
      { id: rid('p'), name: 'products', widgets: [
        mkWidget('text',  20, 20, 400, 40,  { content: '# Products' }),
        mkWidget('table', 20, 70, 760, 360, { dataSource: 'products', sortable: true }),
      ]},
    ];
  } else {
    throw new Error('unknown template: ' + name + ' (choose from: ' + TEMPLATE_NAMES.join(', ') + ')');
  }
  return base;
}

// Route a natural-language prompt to a template name (or null).
export function routePrompt(prompt) {
  const q = String(prompt || '');
  for (const r of TEMPLATE_KEYWORDS) if (r.kw.test(q)) return r.name;
  return null;
}

// ─── Markdown -> HTML (tiny subset, verbatim) ───────────────────────
...
export function validateApp(app) {
  const errs = [];
  if (!app || typeof app !== 'object') return ['app must be an object'];
  if (!app.name) errs.push('app.name is required');
  if (!Array.isArray(app.pages) || !app.pages.length) errs.push('app.pages must be a non-empty array');
  (app.pages || []).forEach((p, i) => {
    if (!p.name) errs.push('page[' + i + '].name is required');
    if (!Array.isArray(p.widgets)) errs.push('page[' + i + '].widgets must be an array');
    (p.widgets || []).forEach((w, j) => {
      if (!WIDGETS[w.type]) errs.push('page[' + i + '].widgets[' + j + '].type invalid: ' + w.type);
    });
  });
  if (!Array.isArray(app.dataSources)) errs.push('app.dataSources must be an array');
  return errs;
}
...
export function renderStandaloneHtml(app, opts) {
  opts = opts || {};
  const bake = opts.bakeDemoData !== false;
  const data = {};
  if (bake) {
    (app.dataSources || []).forEach(ds => {
      if (ds.type === 'demo' && DEMO_DATA[ds.demo]) data[ds.name] = DEMO_DATA[ds.demo];
    });
  }
  return `<!doctype html>
<html lang="en">
...
<div id="app"></div>
<script>
window.__FALLBUILD_APP__=${JSON.stringify(app)};
window.__FALLBUILD_DATA__=${JSON.stringify(data)};
${RUNTIME_SRC}
<\/script>
</body>
</html>`;
...
export function renderTemplateHtml(name, opts) {
  return renderStandaloneHtml(applyTemplate(name), opts);
}

export const VERSION = '1.0.0';

export default {
  VERSION, WIDGETS, DEMO_DATA, TEMPLATE_NAMES, TEMPLATE_KEYWORDS,
  createApp, addPage, addWidget, addDataSource, mkWidget,
  applyTemplate, routePrompt, validateApp, mdToHtml, esc,
  renderStandaloneHtml, renderTemplateHtml,
};
